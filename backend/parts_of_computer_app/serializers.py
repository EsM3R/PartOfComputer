...
class ProductSerializer(serializers.ModelSerializer):
    class Meta:
        model = Product
        fields = ("name","price","image_url","category")


class MotherBoardFeatureSerializer(serializers.ModelSerializer):
    class Meta:
        model = MotherboardFeature
        fields = ("processor_Socket_Type" , "ram_Type" , "compatible_Processors",  "motherboard_Size"  , "product")
        

class ComputerCaseFeatureSerializer(serializers.ModelSerializer):
    class Meta:
        model =  ComputerCaseFeature
        fields =  ("case_Type", "PSU" ,  "PSU_Location" , "transparent_Case" ,  "type_C" , "product")

class GraphicsCardFeatureSerializer(serializers.ModelSerializer) : 
    class Meta :
        model = GraphicsCardFeature
        fields =  ("gpu_Manufacturer" , "gpu_Model" , "memory_Type" , "gpu_Memory_Capacity" , "product")


class  ProcessorFeatureSerializer(serializers.ModelSerializer) :
    class Meta:
        model =  ProcessorFeature
        fields =  ("processor_Model" , "processor_Manufacturer" , "processor_Socket_Type" , "processor_Series" , "core_Count","product")
...
class CaseFanFeatureSerializer(serializers.ModelSerializer):
    class Meta :
        ...
        fields =  ("cooling_Type" , "fan_Count" , "led_Type" , "power_Connector" , "rpm","product")
        
class KeyboardFeatureSerializer(serializers.ModelSerializer):
      class Meta :
        model =  KeyboardFeature
        fields =  ("connection_Type" , "mechanical" , "keyboard_Layout" , "wrist_Support" , "numpad","product")


class MonitorFeatureSerializer(serializers.ModelSerializer):
      class Meta :
        model =  MonitorFeature
        fields =  ("screen_Size" , "resolution" , "refresh_Rate" , "panel_Type" , "response_Time","product")
    

class MouseFeatureSerializer(serializers.ModelSerializer):
      class Meta :
        model =  MouseFeature
        fields =  ("connection_Type" , "tracking_Type" , "button_Count" , "usage_Type" , "max_DPI" , "product")
    
class RamFeatureSerializer(serializers.ModelSerializer):
      class Meta :
        model =  RamFeature
        fields =  ("ram_Type" , "ram_Capacity" , "ram_Frequency" , "channel_Type" , "ram_Compatibility" , "product")

class CoolerFeatureSerializer(serializers.ModelSerializer):
      class Meta :
        model =  CoolerFeature
        fields =  ("compatible_Sockets" , "cooling_Type" , "led" , "radiator_Size" , "fan_Count" , "product")
